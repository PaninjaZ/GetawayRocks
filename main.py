...
sys.path.insert(0,Path+'data/scripts')
...
audio_volume_define = settings.options[0][1]
ball_speed_current = settings.options[4][1]

fps = settings.options[5][1]
fpsmulti = fps/30
...
def splay(var, chnl = 0):
    if chnl == 0:
        ...
        channel.set_volume(audio_volume_define/100/2)
        channel.play(sound, loops=0)
    else:
        CUSTOMchannel = pygame.mixer.Channel(chnl)
        sound = pygame.mixer.Sound(Path+"data/sounds/"+str(var))
        CUSTOMchannel.set_volume(audio_volume_define/100/2)
...
songchannel.set_volume((audio_volume_define/100)/4)
...
settingsgruppe = pygame.sprite.Group()
...
class Spiller(pygame.sprite.Sprite):
    ...
    def flyttUp(self):
        ...
        self.fartud -= (settings.options[1][1]/fpsmulti/self.speedup)
    ...
    def flyttR(self):
        ...
        self.fartlr += (settings.options[1][1]/fpsmulti/self.speedup)
...
class Ball(pygame.sprite.Sprite):
    ...
    def flytt(self):
        self.rect.x += self.fartx//fpsmulti
        veggtreffliste = pygame.sprite.spritecollide(self, wallgruppe, False)
        ...
        if veggtreffliste:
            ...
            if random.randint(0,3) == 0:
                self.farty *= -1
        self.rect.y += self.farty//fpsmulti
        veggtreffliste = pygame.sprite.spritecollide(self, wallgruppe, False)
...
def spawnBalls(remove = 0):
    ...
    for y, rad in enumerate(level[levelnum]):
        for x, verdi in enumerate(rad):
            if verdi == 7:
                ball = Ball(x*32,y*32, random.randint(settings.options[4][1]*-1, settings.options[4][1]), random.randint(settings.options[4][1]*-1, settings.options[4][1]))
                ballgruppe.add(ball)
...
def fullscreen():
    ...
    if settscreen:
        for stn in settingsgruppe:
            stn.kill()
        settingsgruppe.add(settings.show(Path,width,height))
        settings.show_elements(Path,width,height,settingsgruppe)

    settings.options[3][1] = 1
...
while running:
    ...
    lastDigit = int(repr(((scoretwo % 10)/2) % 10)[-1])
    lastDigitReal = int(repr(scoretwo % 10)[-1])
    ...
    if timeuntilspawnball > timewaituntilspawn and settings.options[2][1] == 1:
    ...
    if lastDigit == 5:
        settings.refresh_elements(settingsgruppe)
    ...
    text(str(clock.get_fps()//1),(35), (height-25), (100, 200, 100), 30)

    if settscreen:
        settingsgruppe.draw(screen)

    if settscreen:
        settings.text(settingsgruppe, 30, Path, screen)
    ...
    if lastDigit == 5:
        ...
        if not ball_speed_current == settings.options[4][1]:
            for ball in ballgruppe:
                ball.kill()
        ...
        if fscreen == False and settings.options[3][1] == 1:
            ...
            fscreen = True
            fullscreen()
        elif fscreen == True and settings.options[3][1] == 0:
            width = round(pygame.display.get_desktop_sizes()[0][0] / 2)
            height = round(pygame.display.get_desktop_sizes()[0][1] / 2)
            pygame.display.toggle_fullscreen()
            print("Setting up window with the size... " + str(width) + " " + str(height))
    ...
    text(('Score: ' + str(int(score/fps))), (100), (50), (200, 200, 200), 30)
                ...
                for y, rad in enumerate(custommap):
                    for x, verdi in enumerate(rad):
                        ...
                        if verdi == 1:
                            ...
                            ball = Ball(x*32,y*32, random.randint(-8, 8), random.randint(-8, 8))
                            ballgruppe.add(ball)
